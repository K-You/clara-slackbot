	...
	try{
		...
		var callback = function(response){
			...
			response.on('end', function(){
				...
				self.postMessageToChannel(channel.name, "Ce site a répondu en "+Date.now()-start+"ms.");
		...
		if(url.indexOf("https")>-1){
			...
			self.postMessageToChannel(channel.name, "Veuillez me préciser le protocole (HTTP/HTTPS)", {as_user:true});
		}
